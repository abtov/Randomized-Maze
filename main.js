...
//console.log(moves);
//console.log(node[2][1][1] + (distanceBetweenNodes / 2));

//console.log(wallCount[wallCount.length-1][3])

/*
@@ MAZE ALGORITHM IDEAS:
1. Each wall is expanding in a different rate.
2. It would multiply paths in a random time
3. If 2 walls collide in the way, it would have a high chance for one to be blocked for other to continue, but a low chance to cross.
4. Continue until it reaches the ending.
5. If it has multiple paths, it would try to calculate which one has the shortest route to the ending and shut it down.

pros:
- It would reduce the chance to have multiple ending routes.
- Has a harder puzzle to solve.
- Unpredictable.

Cons:
- It is time based algorithm (it will load more slowly)

*/ 

// @@ REMEMBER!!: Coordinates starts from top left then go down and switch to right lane.


//console.log(wallCount);

//console.log(wallCount[wallCount.length - 1])
...
1. Trying to save each node's coordinates to be used to connect lines between each nodes. 

2. Make a pernament coordinate rules. 
 - Middle of each 4 nodes is a coordinate. 
 - Make borders as a "Closed" category coordinate.  Done
 - Make spaces as a "Open" category coordinate.     Done
.

3. Testing out walls measurements and adding walls per node counter. Done

4. Making a playable character
*/

/*
@ Game Ideas & Features!

1. Movable & interactable players with controls.
2. A winning system.
3. Collision to walls
4. Animation to player's movements (Bump off walls, take moves)
5. Interactable menu
6. Players breaking walls (animation & particles of breaking introduction).
7. Player's head movements depends on the mouse.
8. Randomized wall placements.                                              Done
9. Score system.
10. Labyrinth system. (Entirely different maze design *circle)

*NEW THOUGHTS*
11. break points (walls that could only be broken), and break point wall color
12. limit visibility to see the break point until it is close by
